"""Gateway-backed storage for liquidity intents with local fallback."""
...
LIQUIDITY_SESSION_ENTITY = "liquidity-sessions"
LIQUIDITY_HISTORY_ENTITY = "liquidity-histories"
...
class LiquidityStateRepository:
    """Stores liquidity agent state via Panorama's gateway or an in-memory fallback."""

    _instance: "LiquidityStateRepository" | None = None
    ...
    def _fallback_to_local_store(self) -> None:
        if self._use_gateway:
            self._logger.warning("Panorama gateway unavailable for liquidity state; switching to in-memory fallback.")
        self._use_gateway = False
        self._init_local_store()

    def _handle_gateway_failure(self, exc: PanoramaGatewayError) -> None:
        self._logger.warning(
            "Panorama gateway error (%s) for liquidity repository: %s",
    ...
    @classmethod
    def instance(cls) -> "LiquidityStateRepository":
    ...
    def get_metadata(self, user_id: str, conversation_id: str) -> Dict[str, Any]:
        ...
        metadata["pool_id"] = intent.get("pool_id")
        metadata["amount"] = intent.get("amount")
        metadata["network"] = intent.get("network")
        metadata["protocol"] = intent.get("protocol")
        metadata["token_a"] = intent.get("token_a")
        metadata["token_b"] = intent.get("token_b")
    ...
    def get_history(
        ...
        try:
            ...
            self._logger.warning("Invalid liquidity history response from gateway; falling back to local store.")
            self._fallback_to_local_store()
            return self.get_history(user_id, conversation_id, limit)
            ...
            history_list.append(
                {
                    ...
                    "pool_id": entry.get("poolId"),
                    "amount": entry.get("amount"),
                    "network": entry.get("network"),
                    "protocol": entry.get("protocol"),
                    "token_a": entry.get("tokenA"),
                    "token_b": entry.get("tokenB"),
    ...
    def _get_session(self, user_id: str, conversation_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            return self._client.get(LIQUIDITY_SESSION_ENTITY, identifier)
        except PanoramaGatewayError as exc:
            if exc.status_code == 404:
                return None
            self._handle_gateway_failure(exc)
    ...
    def _delete_session(self, user_id: str, conversation_id: str) -> None:
        identifier = _identifier(user_id, conversation_id)
        try:
            self._client.delete(LIQUIDITY_SESSION_ENTITY, identifier)
        except PanoramaGatewayError as exc:
            if exc.status_code != 404:
                self._handle_gateway_failure(exc)
                raise
    ...
    def _upsert_session(
        ...
        try:
            self._client.update(LIQUIDITY_SESSION_ENTITY, identifier, payload)
        except PanoramaGatewayError as exc:
            if exc.status_code != 404:
                self._handle_gateway_failure(exc)
                raise
            ...
            try:
                self._client.create(LIQUIDITY_SESSION_ENTITY, create_payload)
    ...
    def _create_history_entry(
        ...
        history_payload = {
            ...
            "poolId": summary.get("pool_id"),
            "amount": _as_float(summary.get("amount")),
            "network": summary.get("network"),
            "protocol": summary.get("protocol"),
            "tokenA": summary.get("token_a"),
            "tokenB": summary.get("token_b"),
            "errorMessage": summary.get("error"),
            "recordedAt": _utc_now_iso(),
            "tenantId": self._tenant_id(),
        }
        try:
            self._client.create(LIQUIDITY_HISTORY_ENTITY, history_payload)
        except PanoramaGatewayError as exc:
            if exc.status_code == 404:
                self._handle_gateway_failure(exc)
                raise
