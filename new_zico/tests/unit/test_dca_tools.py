...
def test_fetch_strategy_suggestions(dca_tools):
    ...
    with tools.dca_session("user-test", "conversation-test"):
        result = tools.fetch_dca_strategy_tool.invoke({"from_token": "USDC", "to_token": "AVAX", "cadence": "daily"})
...
def test_update_dca_flow(dca_tools):
    ...
    with tools.dca_session("user-flow", "conversation-flow"):
        step1 = tools.update_dca_intent_tool.invoke({
        ...
        step2 = tools.update_dca_intent_tool.invoke({
            "cadence": "daily",
            "start_on": "2024-06-10",
            "iterations": 10,
        ...
        final = tools.update_dca_intent_tool.invoke({"confirm": True})
