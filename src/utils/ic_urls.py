...
def canister_candid_ui_url(canister_id: str, base_url: str = "http://127.0.0.1:4943") -> str:
    """
    Formata URL para a interface Candid UI do canister.
    ...
    Returns:
        URL da interface Candid
    """
    if "localhost" in base_url or "127.0.0.1" in base_url:
        port = "4943"
        if ":" in base_url:
            port = base_url.split(":")[-1]
        return f"http://127.0.0.1:{port}/?canisterId={canister_id}"
    else:
        return f"https://a4gq6-oaaaa-aaaab-qaa4q-cai.raw.icp0.io/?id={canister_id}"
